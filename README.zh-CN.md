...
面向生产环境的文档与发票处理工具链：统一 CLI、Cloudflare D1 结构化存储、ChromaDB 语义检索、以及可观测的健康检查。
...
- **文档处理**
  ...
  - 提取文本写入 ChromaDB
- **运维健康检查**
  - 校验 Python 运行时、依赖、D1、ChromaDB、新闻 cron wrapper、Hermes 投递链路
  - 导出 JSON / Markdown 报告
  - 用 `doctor --fix` 修复常见漂移
- **资讯推送工具**
  - Reuters 摘要推送
  - Bloomberg 摘要推送
  - 输出与 Chroma 元数据都带来源增强信息（媒体名 / 域名 / feed）
...
- `CHROMA_HOST`
- `CHROMA_PORT`
- 如果你要用 Hermes 做探针/推送，再填写 `HERMES_NEWS_TARGET`
